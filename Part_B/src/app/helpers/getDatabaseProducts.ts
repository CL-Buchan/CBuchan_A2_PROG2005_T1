import { DatabaseItem } from '../types/types.js';
...
export function getDatabaseProducts(target?: string, errorMsg?: string) {
  ...
  try {
    if (target || errorMsg) {
      const error = document.getElementById(errorMsg || '') as HTMLElement;
      const list = document.querySelector(target || '') as HTMLUListElement;

      if (!error || !list) {
        console.warn('Selected elements cannot be found in document.');
        return [];
      }
      ...
      if (!products) {
        console.error('Returned early as products array does not exist');
        return [];
      }
